...
const Product = () => {
  ...
  const { products , currency, priceMultiplier, addToCart} = useContext(ShopContext);
      ...
      <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
       {
        productData.image.map((item,index)=>(
          <img onClick={()=>setImage(item)} src={item} key={index} className={`w-[24%] sm:w-full sm:mb-3 flex-shrink-0 cursor-pointer ${item === image ? 'border-2 border-orange-500' : ''}`}  alt="" />
        ))
       }
      </div>
    {/* Main Image */}
          ...
          {/*-------Product Info-------- */}
           <div className='flex-1'>
            ...
            <p className='mt-5 text-3xl font-medium'>{currency} {productData.price * priceMultiplier}</p>
